...
import {
  LayoutDashboard, Users, Settings, BarChart3, FileText, Bell, LogOut, Menu, X, ChevronRight,
} from "lucide-react";
...
const navKeys = [
  ...
  { icon: Users, key: "nav.users", path: "/users" },
  { icon: FileText, key: "nav.projects", path: "/projects" },
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm hover:bg-accent transition-colors">
                ...
                <span className="flex-1 truncate text-left text-muted-foreground">{user?.email}</span>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-56">
              <DropdownMenuItem asChild>
                <Link to="/settings">{t("settings.account_settings")}</Link>
              </DropdownMenuItem>
